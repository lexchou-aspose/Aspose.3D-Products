...
description: ASE 到 FBX C# 轉換的示例代碼。 在VB.NET,asp.NET 或任何基於 .NET 的應用程序中使用 API 示例代碼將批處理 ASE 文件轉換為 FBX。
---
...
{{% blocks/products/pf/agp/text %}}
 ...
 .NET 程序員只需幾行代碼就可以輕鬆地將 ASE 文件加載和轉換為 FBX。

{{% /blocks/products/pf/agp/text %}}

1. 通過場景類的構造函數加載 ASE 文件1. 創建FbxSaveOptions實例1. 為高級轉換設置 FBX 特定屬性1. 調用場景。 保存方法1. 傳遞帶有 FBX 文件擴展名的輸出路徑 & FbxSaveOptions的對象1. 檢查指定路徑處的結果 FBX 文件
...
{{% blocks/products/pf/agp/code-block title="此示例代碼顯示 ASE 到 FBX C# 的轉換" offSpacer="" %}}
...
{{< blocks/products/pf/agp/i18n/demobox-app sectionTitle="免費應用程序將 ASE 轉換為 FBX" sectionDescription="查看我們的實時演示 [ASE 到 FBX 轉換](https://products.aspose.app/3d/conversion/ase-to-fbx) 有以下好處。" >}}
