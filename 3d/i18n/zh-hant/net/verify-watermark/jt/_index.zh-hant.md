...
{{% blocks/products/pf/agp/text %}}
 ...
 Aspose.3D 使開發人員只需幾行代碼即可輕鬆地將盲水印驗證添加到 JT 文件中。

{{% /blocks/products/pf/agp/text %}}

- 通過 Scene 類的構造函數加載 JT 文件- 獲取 Aspose.3D 的網格類- 使用 Aspose.3D 的 DecodeWatermark 方法添加密碼- 使用對象調用 Scene.Save 方法
...
{{% blocks/products/pf/agp/code-block title="用於向 JT 添加盲水印驗證的 C# 代碼" offSpacer="" %}}
...
<!-- aboutfile Starts -->
 ...
 Aspose.3D 是用於加載、修改和轉換 3D 文件的 CAD 和遊戲軟件 API。 API 是獨立的，不需要任何 3D 建模或渲染軟件。可以輕鬆地將 API 用於 Discreet3DS、WavefrontOBJ、STL（ASCII，二進制）、Universal3D、FBX（ASCII，二進制）、Collada、glTF、PLY、 GLB、DirectX 和更多格式。 
  ...
  {{< blocks/products/pf/agp/i18n/demobox-app sectionTitle="向 JT 添加盲水印驗證的免費應用程序" sectionDescription="查看我們的現場演示 [水印驗證 JT](https://products.aspose.app/3d/verify-watermark/jt) 具有以下好處。" >}}
