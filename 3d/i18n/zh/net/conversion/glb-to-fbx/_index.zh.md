...
description: GLB 到 FBX C# 转换的示例代码。在VB.NET，Asp.NET 或任何基于 .NET 的应用程序中使用 API 示例代码将批处理 GLB 文件转换为 FBX。
---
...
{{% blocks/products/pf/agp/text %}}
 ...
 .NET 程序员只需几行代码就可以轻松地将 GLB 文件加载和转换为 FBX。

{{% /blocks/products/pf/agp/text %}}

1. 通过场景类的构造函数加载 GLB 文件1. 使用 FBX 的格式调用Scene.Save方法。1. 检查指定路径处的结果 FBX 文件
...
{{% blocks/products/pf/agp/code-block title="此示例代码显示 GLB 到 FBX C# 的转换" offSpacer="" %}}

```cs
// 加载源二进制文件 GLTF
Scene scene = new Scene("sourceFile.glb");
// 将 3D 场景转换为Autodesk FBX 格式的文件
...
{{< blocks/products/pf/agp/i18n/demobox-app sectionTitle="免费应用程序将 GLB 转换为 FBX" sectionDescription="查看我们的实时演示 [GLB 到 FBX 转换](https://products.aspose.app/3d/conversion/glb-to-fbx) 有以下好处。" >}}
